...
def Initial_incident_Road() -> Tuple[List[List[float]], List[List[float]], float, float, int]:
    ...
    if Number_of_Incident_Roads == 1:
        Incident_Road_Temp.append([list((x0 + 3 * delta_L_x ) + t * delta_x), list((y0 + 3* delta_L_y ) + t * delta_y)])
    if Number_of_Incident_Roads == 2:
        Incident_Road_Temp.append([list((x0 + 3 * delta_L_x ) + t * delta_x), list((y0 + 3 * delta_L_y ) + t * delta_y)])
        Incident_Road_Temp.append([list((x0 + 3 * delta_R_x ) + t * delta_x), list((y0 + 3* delta_R_y ) + t * delta_y)])
...
def Incident_Road_Gen(Number_of_incident_Roads: int) -> List:
    ...
    while i < Number_of_incident_Roads:
        ...
        if Number_of_Incident_Roads == 1:
            temp.append([list((x0 + 3 * delta_L_x ) + t * delta_x), list((y0 + 3* delta_L_y ) + t * delta_y)])
        if Number_of_Incident_Roads == 2:
            temp.append([list((x0 + 3 * delta_L_x ) + t * delta_x), list((y0 + 3 * delta_L_y ) + t * delta_y)])
            temp.append([list((x0 + 3 * delta_R_x ) + t * delta_x), list((y0 + 3 * delta_R_y ) + t * delta_y)])
