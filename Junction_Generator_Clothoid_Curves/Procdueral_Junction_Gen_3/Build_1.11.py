...
def Initial_incident_Road() -> Tuple[List[List[float]], List[List[float]], float, float, int]:
    ...
    Number_of_Incident_Roads = random.randint(0, Max_num_Incident_Roads - 1)  # 1, 2 or 3 new lanes

    Partition_of_Inicident_Roads = np.random.uniform(Partition_of_Inicident_Roads_min, Partition_of_Inicident_Roads_max)


    k = random.randint(0,1)
    if Random_Incident_Partition == True:
    ...
    elif Random_Incident_Partition == False:
        A = np.random.uniform(lane_width_min,lane_width_max) + Partition_of_Inicident_Roads
...
#def Boundry_gen(Incident_boundry, Incident_road_index, Incident_Roads):
    Roads, I = Incident_boundry, Incident_road_index
    Connection_Roads = []
    Junction_Boundry =[]
    ...
    for j in range(len(Incident_Roads)):
        ...
        if Incident_Roads[I][-1] == 3 and Incident_Roads[j][-1] == 3:
            ...
            Junction_Boundry.append(Clothoid_Curve(Roads[I][1][0][0][0], Roads[I][1][0][1][0], Roads[j][2][1][0][0], Roads[j][2][1][1][0], Roads[I][0][-1], Roads[j][0][-2]))
            Junction_Boundry.append(Clothoid_Curve(Roads[I][2][1][0][0], Roads[I][2][1][1][0], Roads[j][1][0][0][0], Roads[j][1][0][1][0], Roads[I][0][-1], Roads[j][0][-2]))
...
def main():    
    ...
    #for i in range(len(Incident_Roads)):
        #Junction_Boundry =  Junction_Boundry  +  Boundry_gen(Incident_Roads_boundry,i,Incident_Roads)  
...
Num_Incident_Roads = 4

Curvature_of_Incident_Road_placment = 0.020   ### recommend 0.025

Max_num_Incident_Roads = 3                  ### max 3 min 2...
...
Random_Incident_Partition = True         
