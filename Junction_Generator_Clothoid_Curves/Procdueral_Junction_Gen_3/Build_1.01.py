...
def Incident_Road_Gen(Number_of_incident_Roads: int) -> List:
    ...
    while i < Number_of_incident_Roads:
        Helper_paths.append(Helper_Path_Gen(Roads[i-1][0][0], Roads[i-1][1][0], Roads[i-1][3], 10))
        ...
        for j in range(len(Roads) - 1):
            if np.abs(Roads[-1][0][0] - Roads[j][0][0]) <= 2.5 and np.abs(Roads[-1][1][0] - Roads[j][1][0]) <= 2.5:
                Roads.pop()
                Helper_paths.pop()
                should_pop = True
                break
...
def Clothoid_Curve(a, b, c, d, heading0, heading1):
    ...
    def clothoid_curve(x0, y0, theta0, L, kappa0, kappa1, num_points=1000):
        ...
        for i, s in enumerate(s_vals):
            x_vals[i] = x0 + np.cos(0.5 * kappa1 * s**2 + kappa0 * s + theta0) * s
            y_vals[i] = y0 + np.sin(0.5 * kappa1 * s**2 + kappa0 * s + theta0) * s
...
Incident_Roads = Incident_Road_Gen(2)

for i in range(len(Incident_Roads)):
    for j in range(len(Incident_Roads)):
        Connection_Roads.append(Clothoid_Curve(Incident_Roads[i][0][0],Incident_Roads[i][1][0],Incident_Roads[j][0][0],Incident_Roads[j][1][0],Incident_Roads[i][3],Incident_Roads[j][2]))

Connection_road_plotter(Connection_Roads)

for i in range(len(Incident_Roads)):
    Incident_Plotter(Incident_Roads[i][0], Incident_Roads[i][1])
    plt.plot(Incident_Roads[i][0][0], Incident_Roads[i][1][0], 'ro')
